/**
 * Patches src/db/schema.ts after Better Auth CLI generate.
 * Injects recommended indexes into generated Drizzle tables so they are not
 * lost on regenerate. All index names are prefixed with "admin_" to avoid
 * conflicts with the main web app's tables in the shared database.
 *
...
const AUTH_SCHEMA_PATH = join(ROOT, 'src/db/schema.ts');

const EXPORT_PREFIX = 'export const ';

/** Recommended indexes per table variable name. All index names use the admin_ prefix. */
const TABLE_INDEXES: Record<string, Array<string>> = {
  user: [
    'index("admin_user_createdAt_idx").on(table.createdAt)',
    'index("admin_user_emailVerified_idx").on(table.emailVerified)',
    'index("admin_user_banned_idx").on(table.banned)',
    'index("admin_user_lastSignInAt_idx").on(table.lastSignInAt)',
  ],
  session: [
    'index("admin_session_userId_idx").on(table.userId)',
...
/** Extract index id from a line like index("admin_user_createdAt_idx").on(...). */
