# Design: Add Standalone Hono API in `apps/api`
...
Add a new standalone Hono server in `apps/api` with a single `GET /hello` endpoint for smoke testing. The new app should follow the monorepo's workspace conventions, include essential HTTP middlewares, and be runnable, type-checkable, and lintable as a first-class app.
...
- Create a new workspace app at `apps/api`.
...
| File                                        | Responsibility                                                                                        |
| ------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `apps/api/src/app.ts`                       | Create the Hono app, register global middleware, mount routes, and attach not-found / error handling. |
| `apps/api/src/index.ts`                     | Start the Node HTTP server on the configured port.                                                    |
| `apps/api/src/routes/hello.ts`              | Define the `GET /hello` test endpoint.                                                                |
| `apps/api/src/lib/env.ts`                   | Parse and expose server environment values such as port and CORS origin.                              |
| `apps/api/src/middleware/request-id.ts`     | Ensure each request has a stable request ID attached to the context and response headers.             |
| `apps/api/src/middleware/request-logger.ts` | Log request/response metadata, including request ID and duration.                                     |
| `apps/api/src/middleware/error-handler.ts`  | Normalize thrown errors into JSON responses for API consumers.                                        |
...
`apps/api` should be a workspace package with scripts aligned to the existing apps, adapted for a server-only runtime:
...
- `apps/api/tsconfig.json` should extend the root TypeScript config like the other apps.
...
1. `apps/api` is recognized as a workspace app.
