# Design: Add Standalone Hono API in `apps/appfolio-api-svc`
...
Add a new standalone Hono server in `apps/appfolio-api-svc` with a single `GET /hello` endpoint for smoke testing. The new app should follow the monorepo's workspace conventions, include essential HTTP middlewares, and be runnable, type-checkable, and lintable as a first-class app.
...
- Create a new workspace app at `apps/appfolio-api-svc`.
...
| File                                                     | Responsibility                                                                                        |
| -------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| `apps/appfolio-api-svc/src/app.ts`                       | Create the Hono app, register global middleware, mount routes, and attach not-found / error handling. |
| `apps/appfolio-api-svc/src/index.ts`                     | Start the Node HTTP server on the configured port.                                                    |
| `apps/appfolio-api-svc/src/routes/hello.ts`              | Define the `GET /hello` test endpoint.                                                                |
| `apps/appfolio-api-svc/src/lib/env.ts`                   | Parse and expose server environment values such as port and CORS origin.                              |
| `apps/appfolio-api-svc/src/middleware/request-id.ts`     | Ensure each request has a stable request ID attached to the context and response headers.             |
| `apps/appfolio-api-svc/src/middleware/request-logger.ts` | Log request/response metadata, including request ID and duration.                                     |
| `apps/appfolio-api-svc/src/middleware/error-handler.ts`  | Normalize thrown errors into JSON responses for API consumers.                                        |
...
`apps/appfolio-api-svc` should be a workspace package with scripts aligned to the existing apps, adapted for a server-only runtime:
...
- `apps/appfolio-api-svc/tsconfig.json` should extend the root TypeScript config like the other apps.
...
1. `apps/appfolio-api-svc` is recognized as a workspace app.
