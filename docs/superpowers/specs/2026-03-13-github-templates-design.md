...
- External link entry: label "Question / Help", url pointing to GitHub Discussions. URL is a placeholder (`https://github.com/<org>/<repo>/discussions`); substitute the actual repo Discussions URL at implementation time.
...
4. **Conventions** — highlight key rules with link to CLAUDE.md for the complete list:
   ...
   - No `any` types
   - Never manually edit `src/components/ui/` — use `pnpx shadcn@latest add <component>`
   - Never edit `src/routeTree.gen.ts` — it is auto-generated by the router plugin
...
MIT License. Substitute the project author's name and current year (2026) into the standard MIT template at implementation time.

## CODE_OF_CONDUCT.md

Contributor Covenant v2.1, unmodified. The enforcement section requires a concrete contact email — this is a required substitution that must be filled in before the file is usable. Placeholder to be replaced at implementation time.
